...
markdown 是备忘录的**唯一内容源**——版本快照存的是它，版本 diff 比的是它，
北大法宝引用核验的输入也是它。再按结构化数据排一版，等于维护两套渲染逻辑，
改了报告结构就得同步改两处，迟早出现「界面上看得到的内容 Word 里没有」。

只支持备忘录实际用到的几种语法（标题 / 列表 / 引用 / 加粗 / 行内代码 / 分隔线）。
...
def markdown_to_docx(markdown: str, *, title: Optional[str] = None,
    ...
    与正文里的 H1 并不一致（例：正文 H1 是「主诉评估决策备忘录：某某案」，
    而导出的文件名只需要后半段）。
    """
    doc = Document()
