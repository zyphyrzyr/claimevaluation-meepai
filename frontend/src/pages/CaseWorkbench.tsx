  ...
  return (
    <div className="max-w-[67rem] mx-auto xl:grid xl:grid-cols-[9rem_minmax(0,1fr)] xl:gap-8">
      {/* 章节导航列：正好填进左侧边栏与内容区之间原先空着的那段留白。
          ...
          mt 与 top 是两个不同的量，必须分开算（写成同一个值就会一滚动就跳位）：
          · mt = calc(25vh+3rem) —— 决定「自然位置」（未滚动时落在哪）。
            自然位置 = main 的 pt-5(20px) + 25vh + 48px = calc(25vh+68px)。
            而侧边栏首项 = py-5(20) + text-xl 行高(28) + py-5(20) + mt-[25vh] = calc(25vh+68px)，
            两者恰好相等，所以 mt 取 3rem 时导航与侧边栏首项是天然对齐的。
          · top = calc(25vh+4.25rem) —— 决定「粘住后停在哪」。必须等于自然位置，
            否则滚动到触发点时会被顶到阈值、瞬间跳一段（差值就是偏移量）。
            原来写成 3rem，比自然位置小 20px，表现就是「一滚就往上蹿 20px 然后停住」。
          实测（headless Chrome 1440x757，侧边栏首项 y=257）：
          改前 scroll=0 时差值为 0、scroll≥20 后恒为 -20；改后全程差值 0，导航纹丝不动。
          行程安全性：只要视口高度 ≥ 341px，导航都不会被包含块底边顶出。
      ...
      <div className="hidden xl:block">
        {activeTab === 'detail' && (
          <SectionNav
            ...
            className="mt-[calc(25vh+3rem)] sticky top-[calc(25vh+4.25rem)]"
          />
        )}
      </div>
