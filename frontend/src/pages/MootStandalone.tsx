...
export default function MootStandalone() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-surface border border-line p-5 space-y-4">
        <div>
          ...
          <textarea
            ...
            className="mt-1.5 w-full border border-line rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-fg"
          />
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="bg-fg text-canvas px-5 py-2 rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            {running ? '庭审进行中…' : '开庭演练'}
          </button>
        </div>
      ...
      {error && <div className="bg-danger-soft text-danger rounded-lg p-4 text-sm">{error}</div>}
      ...
      {report && !running && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 bg-surface border border-line p-5">
            <h2 className="text-sm font-medium mb-3">法官归纳</h2>
            <p className="text-sm leading-relaxed text-muted">{report.judge_summary}</p>
            {report.weak_points.length > 0 && (
              <div className="mt-4">
                <div className="text-xs font-medium text-danger mb-1">我方薄弱点</div>
            ...
            {report.focus_points.length > 0 && (
              <div className="mt-3">
                <div className="text-xs font-medium text-success mb-1">庭前补强建议</div>
          ...
          <div className="bg-surface border border-line p-5">
            <div className="text-xs text-muted">对方抗辩强度</div>
            <div className="mt-1 flex items-center gap-3">
              <div className="flex-1 h-2 bg-line rounded-full overflow-hidden">
                <div className="h-full bg-fg rounded-full" style={{ width: `${report.defense_strength}%` }} />
              </div>
